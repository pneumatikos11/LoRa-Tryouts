...
void do_send(osjob_t *j)
{
  ...
  else
  {
    ...
    //LDR
    photocellReading0 = analogRead(ldrPin);   // Read the analogue pin
    float Vout0 = photocellReading0 * 0.0048828125;  // calculate the voltage
    int lux0 = 500 / (Res0 * ((5 - Vout0) / Vout0)); // calculate the Lux
    Serial.print("Illuminance: ");                 // Print the measurement (in Lux units) in the screen
    Serial.print(lux0);
    Serial.print(" Lux\t");
    float lux = lux0/100.0;
    ...
    payload[2] = humLow;
    payload[3] = humHigh;
    uint16_t payloadLdr = LMIC_f2sflt16(lux);
    byte luxLow = lowByte(payloadLdr);
    byte luxHigh = highByte(payloadLdr);
    payload[4] = luxLow;
    payload[5] = luxHigh;
    //edited
    Serial.print("Pld: ");
